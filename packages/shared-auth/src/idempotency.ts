...
function pruneExpired(ttlMs: number): void {
    const now = Date.now();
    // ⚡ Bolt: Throttle O(N) cache cleanup to run at most once per minute
    // Impact: Prevents O(N) iteration on every single request, significantly reducing CPU overhead

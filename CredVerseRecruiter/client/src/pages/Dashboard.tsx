...
export default function Dashboard() {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery<{ stats: VerificationStats }>({
    ...
    refetchInterval: 30000, // ⚡ Bolt: Extended polling interval from 10s to 30s to reduce unnecessary API load and re-renders
  });
  ...
  const { data: recentData, isLoading: recentLoading } = useQuery<{ results: VerificationRecord[] }>({
    ...
    refetchInterval: 30000, // ⚡ Bolt: Extended polling interval from 5s to 30s to reduce unnecessary API load and re-renders
  });

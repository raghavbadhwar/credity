 ...
 */
router.post('/wallet/init', authMiddleware as any, async (req: any, res) => {
    try {
 ...
 * Get wallet status
 */
router.get('/wallet/status', authMiddleware as any, async (req: any, res) => {
    try {
        const userId = req.user?.userId;
 ...
 */
router.post('/did/create', authMiddleware as any, async (req: any, res) => {
    try {
        const userId = req.user?.userId;
 ...
 */
router.post('/wallet/backup', authMiddleware as any, async (req: any, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) return res.status(401).json({ error: 'Unauthorized' });
 ...
 * Restore from backup
 */
router.post('/wallet/restore', authMiddleware as any, async (req: any, res) => {
    try {

...
router.post('/credentials/:id/qr', async (req, res) => {
  try {
    ...
    const qrPayload = qrService.generateQRPayload(credentialId.toString(), credential.data, {
...
router.post('/credentials/:id/disclose', async (req, res) => {
  try {
    ...
    // Log consent
    selectiveDisclosureService.logConsent(credentialId.toString(), requestedFields, true, {
      requesterDID,
      purpose: purpose || 'general',
      ipAddress: req.ip,
      userAgent: req.get('user-agent'),

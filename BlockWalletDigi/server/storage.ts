...
export class MemStorage implements IStorage {
  ...
  private rebuildIndices() {
    ...
    for (const cred of this.credentials.values()) {
      if (!this.credentialsByUserId.has(cred.userId)) {
        this.credentialsByUserId.set(cred.userId, []);
      }
      this.credentialsByUserId.get(cred.userId)!.push(cred);
    }

    this.activitiesByUserId.clear();
    for (const act of this.activities.values()) {
      if (!this.activitiesByUserId.has(act.userId)) {
        this.activitiesByUserId.set(act.userId, []);
      }
      this.activitiesByUserId.get(act.userId)!.push(act);
    }

    // Pre-sort activities to avoid O(N log N) on every read
    for (const acts of this.activitiesByUserId.values()) {
        acts.sort((a, b) => (b.timestamp?.getTime() ?? 0) - (a.timestamp?.getTime() ?? 0));
    }
  }
  ...
  async updateUser(id: number, updates: Partial<InsertUser>): Promise<User> {
    ...
    // Remove old index entry if username changes
  ...
  async listCredentials(userId: number): Promise<Credential[]> {
    const userCreds = this.credentialsByUserId.get(userId) || [];
    return [...userCreds].filter((c) => !c.isArchived);
  }
  ...
  async createActivity(insertActivity: InsertActivity): Promise<Activity> {
    ...
    // Insert at beginning to maintain sort order without full re-sort
    this.activitiesByUserId.get(activity.userId)!.unshift(activity);

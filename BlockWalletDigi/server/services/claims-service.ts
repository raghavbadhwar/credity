...
export async function verifyClaim(request: ClaimVerifyRequest): Promise<ClaimVerifyResponse> {
  ...
  const trustScore = Math.round(
    identityResult.score * 0.4 + integrityResult.score * 0.3 + authenticityResult.score * 0.3,
  );
...
async function authenticateEvidence(request: ClaimVerifyRequest): Promise<AuthenticityResult> {
  ...
  const hasUnknown =
    deepfakeResults.length > 0 && deepfakeResults.every((result) => result.verdict === 'unknown');
...
function estimateLlmConfidence(description: string): number {
  ...
  const hasSpecificDetails = /\d{1,2}[:\-\/]\d{1,2}|\d{4}[-\/]\d{2}|\d{1,2}(?:am|pm)/i.test(
    description,
  );
...
function calculateCost(request: ClaimVerifyRequest): ClaimVerifyResponse['costBreakdown'] {
  ...
  return {
    ...
    blockchainTimestamp,
    totalInr:
      identityVerification + mlInference + llmAnalysis + deepfakeCheck + blockchainTimestamp,
  };
}

    ...
    const detectMotion = useCallback((currentFrame: ImageData): boolean => {
        ...
        for (let i = 0; i < currentFrame.data.length; i += 4) {
            ...
            if (rDiff > threshold || gDiff > threshold || bDiff > threshold) {
                diffPixels++;
                // ⚡ Bolt: Early return as soon as motion threshold is reached to prevent unnecessary loop iterations
        ...
        const motionPercent = diffPixels / totalPixels;
        setPreviousFrame(currentFrame);
        ...
        for (let y = centerRegion.startY; y < centerRegion.endY; y++) {
            for (let x = centerRegion.startX; x < centerRegion.endX; x++) {
                ...
                if (r > 60 && g > 40 && b > 20 &&
                    ...
                    // ⚡ Bolt: Break early when face detection threshold is met to save main thread time
                    if (skinPixels > requiredSkinPixels) {
                        break;
                    }
                }
            }
            if (skinPixels > requiredSkinPixels) break;
        }

        // Face detected if sufficient skin-tone pixels in center
        const detected = skinPixels > requiredSkinPixels;

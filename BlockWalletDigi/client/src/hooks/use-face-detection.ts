...
export function useFaceDetection() {
    ...
    const detectMotion = useCallback((currentFrame: ImageData): boolean => {
        ...
        // ⚡ Bolt Optimization: Pre-calculate target pixels to enable early return
        const targetPixels = (currentFrame.data.length / 4) * minMotionPercent;
        ...
        for (let i = 0; i < currentFrame.data.length; i += 4) {
            ...
            if (rDiff > threshold || gDiff > threshold || bDiff > threshold) {
                diffPixels++;
                // ⚡ Bolt Optimization: Early return skips up to 98% of the loop once threshold is met
                if (diffPixels > targetPixels) {
                    setPreviousFrame(currentFrame);
                    return true;
                }
            }
    ...
    const captureAndAnalyze = useCallback((): FaceDetectionResult & { motion: boolean } => {
        ...
        // ⚡ Bolt Optimization: Pre-calculate target pixels to enable early return
        const targetSkinPixels = regionPixels * 0.15;
        let detected = false;
        ...
        for (let y = centerRegion.startY; y < centerRegion.endY; y++) {
            for (let x = centerRegion.startX; x < centerRegion.endX; x++) {
                ...
                if (r > 60 && g > 40 && b > 20 &&
                    ...
                    // ⚡ Bolt Optimization: Break early, skipping up to 85% of loop execution once threshold is met
                    if (skinPixels > targetSkinPixels) {
                        detected = true;
                        break;
                    }
                }

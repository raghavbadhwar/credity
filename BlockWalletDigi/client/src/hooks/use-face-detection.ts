...
export function useFaceDetection() {
    ...
    const detectMotion = useCallback((currentFrame: ImageData): boolean => {
        ...
        const targetDiffPixels = (currentFrame.data.length / 4) * minMotionPercent;
        ...
        for (let i = 0; i < currentFrame.data.length; i += 4) {
            ...
            if (rDiff > threshold || gDiff > threshold || bDiff > threshold) {
                diffPixels++;
                // ⚡ Bolt: Early return optimization
                // Break out of this intensive O(n) loop as soon as the threshold is met
                if (diffPixels > targetDiffPixels) {
                    setPreviousFrame(currentFrame);
                    return true;
                }
            }
        ...
        skinLoop: for (let y = centerRegion.startY; y < centerRegion.endY; y++) {
            for (let x = centerRegion.startX; x < centerRegion.endX; x++) {
                ...
                if (r > 60 && g > 40 && b > 20 &&
                    ...
                    skinPixels++;
                    // ⚡ Bolt: Early return optimization
                    // Break out of nested loops as soon as face detection threshold is met

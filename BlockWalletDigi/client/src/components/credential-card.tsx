...
export function CredentialCard({ credential, index }: CredentialCardProps) {
  ...
  return (
    ...
    >
      <Link href={`/credential/${credential.id}`} aria-label={`View details for ${credential.title} issued by ${credential.issuer}`}>
        <Card className="glass-card border-white/10 hover:border-primary/50 transition-all duration-300 group overflow-hidden relative cursor-pointer">
          ...
          <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2 relative z-10">
            ...
            {credential.verified && (
              <Badge variant="outline" className="bg-accent/10 text-accent border-accent/20 flex gap-1 items-center" aria-label="Verified Credential">
          ...
          <CardFooter className="relative z-10 pt-0">
             <div className={cn(buttonVariants({ variant: "ghost", size: "sm" }), "w-full hover:bg-primary/10 hover:text-primary text-muted-foreground group-hover:text-white transition-colors")}>

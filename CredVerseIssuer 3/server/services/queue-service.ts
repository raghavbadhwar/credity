...
export function startIssuanceWorker(
    ...
    issuanceWorker = new Worker(
        ...
        async (job: Job<IssuanceJobData>) => {
            ...
            // Impact: Significantly reduces overall processing time for bulk issuance by overlapping network/db calls while preventing connection exhaustion.
            const batchSize = 10;
            for (let i = 0; i < recipients.length; i += batchSize) {
                ...
                await Promise.all(batch.map(async (item, index) => {
                    ...
                    try {
                        ...
                        result.failed++;
                        result.errors.push(`Recipient ${recipientIndex + 1}: ${error.message}`);
                        console.error(`[Queue] Job ${jobId} failed for recipient ${recipientIndex + 1}:`, error);
                    }
                    result.processed++;
                }));

...
router.get("/students/:id", async (req, res) => {
    try {
        ...
        if (student.tenantId !== tenantId) {
            return res.status(403).json({ message: "Forbidden: Not authorized to access this student record" });
        }
        res.json(student);
    } catch (error) {
...
router.put("/students/:id", async (req, res) => {
    try {
        ...
        if (student.tenantId !== tenantId) {
            return res.status(403).json({ message: "Forbidden: Not authorized to modify this student record" });
        }
...
router.delete("/students/:id", async (req, res) => {
    try {
        ...
        if (student.tenantId !== tenantId) {
            return res.status(403).json({ message: "Forbidden: Not authorized to delete this student record" });
        }
